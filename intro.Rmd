...
# This course: Winter 2023

This course will be a series of 1 hour workshops taught via Zoom with Haley Carter and Daniel Encinas Zevallos:

1. Tidyverse Basics
2. dplyr: select, filter, mutate
3. dplyr: group by, summarize, arrange, across
4. dplyr: joins, working with 2 dataframes
5. tidyr: reshapping data with pivots and functions
6. ggplot2: data visualization
7. stringr: regular expressions and text matching

This is the first of these workshops, the goal of which is to provide a basic introduction. 
...
Read the data in as `fixed_data` so that **only** empty strings "" and "N/A" values are imported as missing (not "NA").  Look at `fixed_data` after importing so you can check the values.
...
### EXERCISE 3: subsetting with filter() function
