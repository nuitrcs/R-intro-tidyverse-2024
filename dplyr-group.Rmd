...
What if we want to do something to each column that is more complicated than a single function?  For example, counting missing values, which you do with `sum(is.na(x))`.  You can write your own function! You can read more about functions [here](https://r4ds.had.co.nz/functions.html) 
...
police %>% 
  ...
  summarize(mean_vehicle_age = mean(vehicle_age),
            sd_vehicle_age = sd(vehicle_age)) 
```
We are getting that warning because it is telling us that all the analysis that comes next or subsequent operation will still be grouping. You can group or ungroup.
...
There's considerable variation here, from 1.1 warnings for every citation given to 2.2 warnings for every citation given. So, relative more warnings to citations for black drivers than hispanic ones.
