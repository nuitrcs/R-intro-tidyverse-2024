...
police %>% select(last_col(offset=1)) %>% names()
```
...
police %>% select(outcome, everything())
```
...
police %>% select(any_of(analysis_vars))

police %>% select(any_of(analysis_vars[1]))

compound_int <-  c("ocimene", "linalool")
```
...
# just to pick a few columns to look at

2023-police$vehicle_year
```
...
```{r, eval=TRUE}
police$department_id

police %>%
  mutate(department_id_number = department_id,
         department_id = "EPD")
...
Remember that when using `mutate()`, you're operating on the entire column at once, so you can't select just a subset of the vector as you would with `[]`.  This means more frequently using functions like `ifelse()` or helper functions such as `na_if()`, `replace_na()`, or `case_when()`.  
